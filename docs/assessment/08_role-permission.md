...
システム管理者
  └─ 組織の作成・管理（全テナントを俯瞰）
     ※ 組織内のデータ（支援対象者・アセスメント等）には原則アクセスしない

組織管理者
  └─ 自組織内の全データを管理

担当者
  └─ 自分が担当する支援対象者のデータのみ操作可能
```
...
| 制限 | 内容 |
|------|------|
| テナントデータへのアクセス | 支援対象者・アセスメント・支援計画など組織内データへのアクセス不可。組織の `organization_id` を含む JWT を発行しないことで制御する |
| 操作対象エンドポイント | `/admin/` プレフィックスのエンドポイントのみ操作可能。一般エンドポイントへのアクセスは `403 Forbidden` |
| 組織の物理削除 | 不可。`is_active = false` による論理削除のみ許可（データ保全のため） |
